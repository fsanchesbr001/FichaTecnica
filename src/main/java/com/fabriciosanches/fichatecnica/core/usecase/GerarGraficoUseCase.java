...
public class GerarGraficoUseCase implements GerarGraficoPort {
    ...
    private static final Color COR_LINHA = new Color(21, 101, 192);
    private static final Color COR_PONTOS = new Color(211, 47, 47);
    ...
    public byte[] gerarGraficoPNG(GraficoPrecoItemDTO dto) throws IOException {
        String nomeItem = normalizarTextoParaGrafico(dto.nomeItem());
        String tituloGrafico = normalizarTextoParaGrafico(dto.titulo());
        logger.info("Gerando grafico PNG para item '{}'", nomeItem);

        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        List<String> labels = dto.labels() != null ? dto.labels() : List.of();
        List<BigDecimal> valores = dto.valores() != null ? dto.valores() : List.of();
        List<EventoInfo> eventos = new ArrayList<>(labels.size());

        for (int i = 0; i < labels.size(); i++) {
            EventoInfo evento = parseEventoInfo(labels.get(i), i);
            eventos.add(evento);
            BigDecimal val = i < valores.size() && valores.get(i) != null ? valores.get(i) : BigDecimal.ZERO;
            dataset.addValue(val, nomeItem, new EventoCategoria(evento.data(), evento.codigoEvento(), i));
        }

        JFreeChart chart = ChartFactory.createLineChart(
                tituloGrafico,
                normalizarTextoParaGrafico("Data"),
                normalizarTextoParaGrafico("Valor (R$)"),
                dataset,
        ...
        rangeAxis.setNumberFormatOverride(java.text.NumberFormat.getCurrencyInstance(new java.util.Locale("pt", "BR")));
        ...
        renderer.setDefaultToolTipGenerator((categoryDataset, row, col) -> {
        ...
        chart.addSubtitle(new TextTitle(nomeItem, new Font("SansSerif", Font.ITALIC, 11)));

        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ChartUtils.writeChartAsPNG(baos, chart, CHART_WIDTH, CHART_HEIGHT);
        byte[] pngBytes = baos.toByteArray();

        logger.info("Grafico PNG gerado com sucesso - {} bytes, {} pontos", pngBytes.length, labels.size());
        return pngBytes;
    }

    @Override
    public byte[] gerarGraficoPizzaPNG(GraficoPizzaDTO dto) throws IOException {
        String nomeProduto = normalizarTextoParaGrafico(dto.nomeProduto());
        logger.info("Gerando grafico de pizza PNG para produto '{}'", nomeProduto);
        ...
        for (int i = 0; i < labels.size(); i++) {
            ...
            dataset.setValue(normalizarTextoParaGrafico(labels.get(i)), valor);
        }

        JFreeChart chart = ChartFactory.createPieChart(
                normalizarTextoParaGrafico("Composicao de Custo - ") + nomeProduto,
                dataset,
                true,
                true,
                false
        ...
        chart.addSubtitle(new TextTitle(
                normalizarTextoParaGrafico("Total de Itens: ") + normalizarTextoParaGrafico(dto.valorTotal()),
        ...
        for (int i = 0; i < labels.size(); i++) {
            if (i < cores.size() && cores.get(i) != null) {
                plot.setSectionPaint(normalizarTextoParaGrafico(labels.get(i)), Color.decode(cores.get(i)));
            }
        }
        ...
        logger.info("Grafico de pizza PNG gerado com sucesso - {} bytes, {} fatias", pngBytes.length, labels.size());
        return pngBytes;
    }

    private EventoInfo parseEventoInfo(String labelOriginal, int fallbackOrdem) {
        String base = normalizarTextoParaGrafico(labelOriginal);
    ...
    private static String normalizarTextoParaGrafico(String texto) {
